...
contract Nominate {
  ...
  {
    require(
      allAwards[_awardId].recipientAddress !=
        allAwards[_awardId].nominatorAddress,
      'this award can not accept any donations until recipient address has been set'
    );
  ...
  function startAwardAndDonate(
    uint256 _awardId,
    address payable _recipientAddress,
    uint256 _donationLimit
  ) public payable aboveMinimum {
    require(
      allAwards[_awardId].active == false &&
        allAwards[_awardId].recipientAddress == address(0),
      'this award has already been added to the smart contract'
    );
  ...
  function setRecipient(uint256 _awardId, address payable _recipientAddress)
    public
    awardExist(_awardId)
  {
    require(
      msg.sender == relayerAddress,
      'msg.sender is not the designated sender for setting recipient'
    );
  ...
  function setRecipients(
    ...
    for (uint256 i = 0; i < _awardIdList.length; i++) {
  ...
  function expireAward(uint256 _awardId) public awardExist(_awardId) {
    // onlyOwner modifier?
    deactivateAward(_awardId);
  }

  // Helpper Functions -----------------------------------------------------------------------------------------------------
  function checkLimit(uint256 _awardId) internal {
    if (
      allAwards[_awardId].donationLimit <= allAwards[_awardId].donationTotal
    ) {
      // emit goal reached
      emit Award_Goal_Reached(
        allAwards[_awardId].recipientAddress,
        address(this),
  ...
  function deactivateAward(uint256 _awardId) public awardExist(_awardId) {
    //donationTotal back to 0
    uint256 amount = allAwards[_awardId].donationTotal;
  ...
  function createAwardStruct(
    uint256 _awardId,
    address _nominatorAddress,
    address payable _recipientAddress,
    uint256 _donationLimit
  ) internal {
  ...
  modifier timeCheck(uint256 _awardId) {
    require(
      block.timestamp > allAwards[_awardId].expires,
      'this contract is not expired and payout will not happen'
    );
    _;
  }
  modifier aboveMinimum() {
    require(
      msg.value > 0 ether,
      'please donate an amount greater than 0 ether'
    );
    _;
  }
  // does award exist?
  modifier awardExist(uint256 _awardId) {
  ...
  event Set_Recipient(address indexed _newRecipient, uint256 _awardId);
}
